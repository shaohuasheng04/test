...
warnings.filterwarnings('ignore')  # 移除警告

IMAGE_DIR = './dataset/WIDER/WIDER_val/images/'
...
class WIDERDataset():
    ...
    def __init__(self):
        ...
        self.split_file = './dataset/WIDER/wider_face_split/wider_face_val_bbx_gt.txt'
...
def main():
    ...
    for image_index, image_name in enumerate(WD.data_dict.keys(), 1):
        ...
        if true_num * positive_num > 0:
            ...
            output += '- | TP = %02d | TN =    |\n' % (true_positive)
            output += '  | FP = %02d | FN = %02d |\n' % (positive_num - true_positive, true_num - true_positive)
            ...
            output += '- Avg.            IoU = {}\n'.format(total_iou / image_index)
            output += '- Avg.         Recall = {}\n'.format(total_recall / image_index)
            output += '- Avg.      Precision = {}\n'.format(total_precision / image_index)
            output += '- Avg.       F1-score = {}\n'.format(total_f1score / image_index)
            output += '- Avg. Inference Time = {}\n'.format(total_time / image_index)
            output += '\n'

            with open('s3fd.txt', 'a') as f:
                f.write(output)

            print('数据保存在out.txt文件中。')
        print()
